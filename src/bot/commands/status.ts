...
import { profileLabel, resolveAccessProfile } from "../../security/access-policy.js";
import { getConfig } from "../../utils/config.js";

const emoji: Record<string, string> = { online: "🟢", idle: "⚪", offline: "🔴" };
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const profile = profileLabel(resolveAccessProfile(interaction.channelId, getConfig()));
  const embed = new EmbedBuilder().setTitle("Session status").setColor(0x7c3aed).setFooter({ text: `Access profile: ${profile}` }).setTimestamp();
