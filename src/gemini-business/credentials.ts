 ...
 *
 * The pool itself is configured in `proxy.yaml`, including the workspace it
 * serves; only the identity-provider secrets live in the environment, so they
 * follow the same rules as every other secret the bot holds: they stay in the
 * bot process and are removed from the Claude subprocess.
 */
